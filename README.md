...
- Одновременно к выходу энкодера автоэнкодера присоединяется **слой кластеризации**, который определяет сходство эмбеддинга с каждым центром кластера.
  Вычисляется - сходство между эмбеддингом точкии центром кластера с помощью t-распределения Стьюдента (также как в t-SNE алгоритме).
  Полученную величину можно интерпретировать как вероятность кластера j для точки i:
    <img src="img/qij.jpg" width=330>
  
  Вычисляется - вероятность вложенной точки  в кластере (p называют целевым распределением):
  ...
  В течении обратного распространения вычисляются градиенты L по  (передаются для обновления (весов автоэнкодера)) и (используются для обновления центров кластеров ).
  кластер объекта 

  общая функция потерь:
    <img src="img/idec_loss.jpg" width=200>

  <br>$qwe$ эффициент контролирующий степень искажения вложенного пространства. Чем больше значение, тем эмбеддинги точек будут более кластеризируемыми и менее надежными (хуже представляют исходные ряды)
